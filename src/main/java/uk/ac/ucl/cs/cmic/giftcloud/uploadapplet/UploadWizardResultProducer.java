            ...
            try {
                ...
                upload = _executorService.submit(new Callable<UploadResult>() {
                    public UploadResult call() {
                        try {
                            UploadResult result = session.uploadTo(false, uploadSelector.getProject().toString(), uploadSelector.getSubject().getLabel(), giftCloudServer, sessionParameters, uploadSelector.getProject(), new SwingUploadFailureHandler(), reporter);
