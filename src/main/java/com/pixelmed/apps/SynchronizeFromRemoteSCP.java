...
public class SynchronizeFromRemoteSCP {
	...
	private static final String identString = "@(#) $Header: /userland/cvs/pixelmed/imgbook/com/pixelmed/apps/SynchronizeFromRemoteSCP.java,v 1.15 2014/09/09 20:34:09 dclunie Exp $";
...
if (verbosityLevel == 2 && ie != null && ie.equals(InformationEntity.STUDY)) System.err.println("Processing study "+node);
if (verbosityLevel > 2) System.err.println("Processing node "+node);
...
if (verbosityLevel > 2) System.err.println("Found "+recordsForThisUID.size()+" existing records");
...
if (verbosityLevel > 1) System.err.print("Retrieve identifier:\n"+retrieveIdentifier);
...
if (verbosityLevel > 2) System.err.println("Child node count "+n);
			if (n > 0) {
				for (int i=0; i<n; ++i) {
...
if (verbosityLevel > 0) System.err.println("Retrieving with C-MOVE");
					...
					if (association != null) {
						moveSOPClassSCU = new MoveSOPClassSCU(association,localAE,SOPClass.StudyRootQueryRetrieveInformationModelMove,retrieveIdentifier,debugLevel);
					}
					else {
						moveSOPClassSCU = new MoveSOPClassSCU(remoteHost,remotePort,remoteAE,localAE,localAE,SOPClass.StudyRootQueryRetrieveInformationModelMove,retrieveIdentifier,debugLevel);
					}
...
if (verbosityLevel > 1) System.err.println("Query "+patientNames[i]);
	...
	/**
	 */
	private void performQueryAndStudyRetrieve() throws DicomException, DicomNetworkException, IOException {
		SpecificCharacterSet specificCharacterSet = new SpecificCharacterSet((String[])null);
		AttributeList identifier = new AttributeList();
		identifier.putNewAttribute(TagFromName.StudyInstanceUID);
...
if (verbosityLevel > 1) System.err.println("Query "+patientNames[i]);
			...
			int n = node.getChildCount();
if (verbosityLevel > 1) System.err.println("Child node count "+n);
			if (n > 0) {
				for (int is=0; is<n; ++is) {
					QueryTreeRecord child = (QueryTreeRecord)node.getChildAt(is);
					if (child != null) {	// should not happen, but has been observed ... do not allow it to derail entire sync :(
if (verbosityLevel > 1) System.err.println("Have child "+child);
...
//if (verbosityLevel > 1) System.err.println("Found "+recordsForThisUID.size()+" existing records");
							AttributeList uniqueKeys = new AttributeList();
							{ Attribute a = new UniqueIdentifierAttribute(uniqueKeyTagFromThisLevel); a.addValue(uid); uniqueKeys.put(a); }
							if (recordsForThisUID.size() == 0) {
if (verbosityLevel > 1) System.err.println("No existing records for "+ie+" "+uid);
if (verbosityLevel > 0) {
	System.err.println("Performing retrieve for "+child+" ("+Attribute.getSingleStringValueOrEmptyString(uniqueKey)+")");
}
...
if (verbosityLevel > 1) System.err.println("Existing records for "+ie+" "+uid+" so retrieving nothing (NOT checking all series or instances)");
							}
						}
					}
				}
...
if (verbosityLevel > 2) System.err.println("Received: "+dicomFileName+" with SOPInstanceUID "+sopInstanceUID);
	...
	public SynchronizeFromRemoteSCP(DatabaseInformationModel databaseInformationModel,File savedInstancesFolder,
		...
		queryInformationModel = new StudyRootQueryInformationModel(remoteHost,remotePort,remoteAE,localAE,debugLevel,reuseAssociations);
		if (retrieveStudy) {
			performQueryAndStudyRetrieve();
		}
		else {
...
if (verbosityLevel > 0) {
	 ...
	 *
	 * @param	arg		array of 7 to 14 strings - the fully qualified path of the database file prefix, the fully qualified path of the saved incoming files folder,
	...
	public static void main(String arg[]) {
		try {
			if (arg.length >= 7 && arg.length <= 14) {
				...
				boolean retrieveStudy = arg.length > 12 ? (arg[12].trim().toUpperCase(java.util.Locale.US).equals("STUDY") ? true : false) : false;
				boolean reuseAssociations = arg.length > 13 ? (arg[13].trim().toUpperCase(java.util.Locale.US).equals("REUSE") ? true : false) : false;
				...
				if (useGet && retrieveStudy) {
					System.err.println("STUDY level retrieval can only be used with MOVE not GET");
					System.exit(0);
				}
				...
				new SynchronizeFromRemoteSCP(databaseInformationModel,savedInstancesFolder,remoteHost,remotePort,remoteAE,localPort,localAE,useGet,queryAll,queryPatternFileName,verbosityLevel,debugLevel,anyTransferSyntax,retrieveStudy,reuseAssociations);
